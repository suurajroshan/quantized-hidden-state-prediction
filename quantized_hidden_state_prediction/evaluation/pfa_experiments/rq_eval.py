...
if __name__ == "__main__":
    ...
    run_ids = ["xseyy20y", "6xui8vbh", "7n8o87u0", "3qhb62nf", "od5oo48n"]

    api = wandb.Api()
    runs = api.runs("hidden-state-predictions/llama-0.1B-rq-layers")
    layers_vs_stats = []
    for run in runs:
        ...
        layers_vs_stats.append({
            ...
            'statistics': stats, 
            'run_id': str(run.id)} )
    with open(f"/home/suurajperpeli/thesis-evaluations/pfa_evals_0.1B/rq-layer-10-llf-1e-4-run-new.json", 'w') as f:
        print('saving run')
